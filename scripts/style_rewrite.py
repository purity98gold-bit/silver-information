...
ANTHROPIC_API_KEY 환경변수가 없으면 조용히 건너뛴다.
"""
import html
import json
import os
...
def main():
    ...
    if not api_key:
        print("ANTHROPIC_API_KEY가 설정되어 있지 않아 스타일 재작성 단계는 건너뜁니다.")
        return
    ...
    try:
        ...
        print("anthropic 패키지가 설치되어 있지 않습니다. requirements.txt를 확인하세요.", file=sys.stderr)
        sys.exit(1)
    ...
    message = client.messages.create(
        ...
        messages=[{"role": "user", "content": build_user_prompt(style_text, recent)}],
    )

    draft_text = "".join(block.text for block in message.content if block.type == "text")
